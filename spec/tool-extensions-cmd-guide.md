...
| `scope` | 否 | `all`（缺省）：所有会话都有这个工具；`scene`：只给**场景**用，普通会话看不到，只有在场景的工具里勾选了它的场景会话或 `/场景命令` 那一轮才会出现（见 `spec/preset-scenes.md`）。`scene` 不能用于替换内置工具 |

配置是部署级的（所有用户、所有会话都一样），改了要重启。`-tools all` 会包含所有扩展工具（`scope: scene` 的除外）；如果用 `-tools a,b,c` 列举，要把扩展工具名也写进去。场景专用工具不受 `-tools` 限制，由场景决定。
